...
def construct_kg(data_dir, train_records, user_set):
    ...
    entity_list = sorted(list(head_set.union(tail_set))) + list(user_set)
...
def get_paths(args):
    ...
    ratings_np = pd.read_csv(data_dir + 'ratings.txt', delimiter='\t', header=None).values
    train_set, eval_set, test_set = data_split(ratings_np, args.ratio)
    item_set = set(ratings_np[:, 1])
    user_set = list(set(ratings_np[:, 0]))
    ...
    rec = get_rec(train_records, eval_records, test_records, item_set, test_set)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='city', help='dataset')
