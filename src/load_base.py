...
def get_rec(train_records, eval_records,  test_records, item_sets, test_set):
    rec = dict()
    users = list(test_records.keys())
    for user in users[:100]:
        neg_items = list(item_sets - set(train_records[user]) - set(test_records[user]))
        if len(neg_items) < 100:
            rec[user] = np.random.choice(neg_items, len(neg_items),  replace=False).tolist() + np.random.choice(test_records[user], 1).tolist()
        else:
            rec[user] = np.random.choice(neg_items, 100, replace=False).tolist() + np.random.choice(test_records[user],1).tolist()
    return rec
