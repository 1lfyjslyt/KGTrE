...
def sampling_paths(data, negative_num, numwalks, size):
    ...
    if os.access("KGTrE/gene/all_negative_samplings", os.F_OK):
        all_negative_samplings = load_data('KGTrE/gene', 'all_negative_samplings')
        all_neighbor_samplings = load_data('KGTrE/gene', 'all_neighbor_samplings')
    else:
        ...
        dump_data(all_neighbor_samplings, 'KGTrE/gene', 'all_neighbor_samplings')
        dump_data(all_negative_samplings, 'KGTrE/gene', 'all_negative_samplings')
    ...
    for i in range(numwalks):
        for p in range(p_num):
            if 1 <= size:
                ...
                u_d.append([p, a+p_num, 1, 5])
                for k in range(negative_num):
                    a_n = all_negative_samplings[3][p].sampling()
                    u_d.append([p, a_n+p_num, -1, 5])
            ...
            if 2 <= size:
                ...
                u_d.append([a+p_num, v+a_num+p_num, 1, 6])
                for k in range(negative_num):
                    v_n = all_negative_samplings[1][p1].sampling()
                    u_d.append([a+p_num, v_n+a_num+p_num, -1, 6])
    ...
    for v in range(v_num):
        if 1 <= size:
            ...
            u_d.append([v+a_num+p_num, p, 1, 1])
            for k in range(negative_num):
                p_n = all_negative_samplings[2][v].sampling()
                u_d.append([v+a_num+p_num, p_n, -1, 1])

            # V-C
            c = all_neighbor_samplings[5][v].sampling()
            u_d.append([v+a_num+p_num, c+v_num+a_num+p_num, 1, 2])
            for k in range(negative_num):
                c_n = all_negative_samplings[5][v].sampling()
                u_d.append([v+a_num+p_num, c_n+v_num+a_num+p_num, -1, 2])
    for c in range(c_num):
        if 1 <= size:
            # C-V
            v = all_neighbor_samplings[4][c].sampling()
            u_d.append([c+v_num+a_num+p_num, v+a_num+p_num, 1, 3])
            for k in range(negative_num):
